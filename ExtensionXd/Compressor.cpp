...
Dictionary* XdCompressor::compress()
{
	...
	CodeHuffman(root, s);
	CompressRatio = CompressorRatio(img, CompressedPixmap);
	DeleteTree(root);
	return dictionary;
}
...
void XdCompressor::HuffmanList(HuffmanNode* &root, ImgWithParam* img)
{
	...
	for (int i = 0; i < img->height; ++i)
	{
		...
		{
			PresentNode = root;
			while (PresentNode != NULL && PresentNode->Color != img->pixmap[i][j]) PresentNode = PresentNode->next;
			if (PresentNode == NULL)
			{
				...
				PresentNode->Color = img->pixmap[i][j];
				PresentNode->count = 0;
				root = PresentNode;
...
bool XdCompressor::CodePixmap(Rgb Color, HuffmanNode* root, std::string code)
{
	if (!root->left)
	{
		...
		{
			CompressedPixmap.push_back(code);
			return true;
		}
	}
...
float XdCompressor::CompressorRatio(ImgWithParam* img, std::vector<std::string> CompressedPixmap)
{
	...
	int CompressedCapacity = CompressedPixmap.capacity();
...
Dictionary* BmpCompressor::compress()
{
	return nullptr;
}